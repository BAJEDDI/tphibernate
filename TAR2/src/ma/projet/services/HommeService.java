...
public class HommeService implements IDao<Homme> {
    ...
    public List<Femme> getEpousesEntreDates(Homme homme, Date d1, Date d2) {
        ...
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            Query query = session.createQuery("getEpousesEntreDates");
...
public static void getEpousesParHomme(Homme homme) {
 ...
 public void afficherHommeMarieParQtrEntreDates(Date startDate, Date endDate) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        ...
        for (Integer hommeId : hommeIds) {
            int marriageCount = countMariagesPourHomme(hommeId, startDate, endDate);
            if (marriageCount == 4) {
    ...
    private int countMariagesPourHomme(int hommeId, Date startDate, Date endDate) {
        Session session = HibernateUtil.getSessionFactory().openSession();
...
public int countHommeMarieParQtrEntreDates(Date startDate, Date endDate) {
    Session session = null;
    int count = 0;
